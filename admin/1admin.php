...
// Fetch all bikes
$sql = "SELECT * FROM `abike`";
$result = $mysqli->query($sql);


// Fetch user details
$booking_results = $mysqli->query("SELECT * FROM abookings  where bike_id like 'GJ%' ORDER BY booking_date");

// Update booking_status from Active to Completed for expired bookings
$sql = "UPDATE abookings
        SET booking_status = 'Completed'
        WHERE booking_status = 'Active' AND return_date <= CURDATE()";

// Handle unpaid status update
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['mark_unpaid'])) {
    // Retrieve the booking_id from POST data
    $booking_id = $_POST['booking_id'];

    if (!empty($booking_id)) {
        // Prepare the SQL query to update booking_status to 'Unpaid'
        $stmt = $mysqli->prepare("UPDATE abookings SET booking_status = 'unpaid' WHERE booking_id = ? AND booking_status = 'active'");
        $stmt->bind_param("s", $booking_id);

        if ($stmt->execute()) {
            echo "Booking successfully marked as Unpaid.";
        } else {
            echo "Error updating booking status to Unpaid: " . $stmt->error;
        }
        $stmt->close();
...
<tr>
    ...
    <form method="POST" action="">
        ...
        <button type="submit" name="mark_unpaid">Unpaid</button>
    </form>
</td>
